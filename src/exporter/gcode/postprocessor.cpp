...
public:
	void preCut(float intensity) { print(m_gcode.preCut(), "S"_a = intensity); }

	void postCut() { print(m_gcode.postCut()); }
	...
	{
		print(m_gcode.cwArcMove(), "X"_a = to.x(), "Y"_a = to.y(), "I"_a = relativeCenter.x(),
			  "J"_a = relativeCenter.y(), "F"_a = feedRate);
	}

	void ccwArcMove(const QVector2D& relativeCenter, const QVector2D& to, float feedRate)
	{
		print(m_gcode.ccwArcMove(), "X"_a = to.x(), "Y"_a = to.y(), "I"_a = relativeCenter.x(),
			  "J"_a = relativeCenter.y(), "F"_a = feedRate);
	}

	void fastPlaneMove(const QVector2D& to) { print(m_gcode.planeFastMove(), "X"_a = to.x(), "Y"_a = to.y()); }

	void retractDepth(float depth) { print(m_gcode.depthFastMove(), "Z"_a = depth); }
	...
	void processLine(const geometry::Bulge& bulge, float planeFeedRate) { planeLinearMove(bulge.end(), planeFeedRate); }
