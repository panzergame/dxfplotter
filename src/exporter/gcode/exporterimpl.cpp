...
module exporter.gcode.exporter;

import config.config;
import config.property;
import config.utils;
import exporter.gcode.metadata;
...
struct ConfigToCommentVisitor
{
	...
	CommentLineStream commentLineStream() const { return CommentLineStream(m_output, m_prefix); }
...
public:
	...
	void start(const QVector2D& from, float safetyDepth) { retractDepth(safetyDepth); }

	void end(const QVector2D& to, float safetyDepth) { fastPlaneMove(to); }
