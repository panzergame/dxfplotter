...
inline IndexStackList convertPathStackToIndexStack(const model::Path::ListPtr& paths, const model::Path::ListPtr& stack)
{
	IndexStackList indexStack(stack.size());
	std::transform(stack.begin(), stack.end(), indexStack.begin(), [&paths](const model::Path* path) {
...
inline model::Path::ListPtr convertIndexStackToPathStack(const model::Path::ListPtr& paths,
														 const IndexStackList& indexStack)
{
...
struct Access<model::Task>
{
	...
	void save(Archive& archive, const model::Task& task, [[maybe_unused]] std::uint32_t const version) const
	{
	...
	template<class Archive>
	void load(Archive& archive, model::Task& task, [[maybe_unused]] std::uint32_t const version) const
	{
		...
		for (const model::Layer::UPtr& layer : task.m_layers) {
			layer->forEachChild([&paths](model::Path& path) {
