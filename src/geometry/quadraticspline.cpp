...
Bezier::List QuadraticSpline::pointsToBeziers(const Point2DList& bezierPoints) const
{
	...
	for (int src = 0, dst = 0; src < (size - 1); src += 2, ++dst) {
		beziers[dst]
			= quadraticBezierPointToCubicBezier(bezierPoints[src], bezierPoints[src + 1], bezierPoints[src + 2]);
	}
...
Bezier::List QuadraticSpline::toBeziers() const
{
	const Point2DList bezierPoints
		= closed() ? convertClosedToQuadraticBezierPoints() : convertOpenedToQuadraticBezierPoints();
