...
private:
	...
	public:
		...
		size_t kdtree_get_point_count() const { return m_tips.size(); }

		float kdtree_get_pt(const size_t idx, const size_t dim) const { return m_tips[idx].point[dim]; }
	...
	Assembler::Tip::List constructTips()
	{
...
public:
	...
	Polyline::List&& polylines() { return std::move(m_mergedPolylines); }
};
