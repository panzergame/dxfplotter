...
private:
	...
	Point2DList convertOpenedToCubicBezierPoints() const
	{
		...
		// Copy of remaining raw points from 2 ... -3 to 3 6 9 ... -10 -7 -4 (every 3 with 6 point distance from begin
		// and end).
	...
	Bezier::List pointsToBeziers(const Point2DList& bezierPoints) const
	{
		...
		for (int src = 0, dst = 0; src < (size - 1); src += 3, ++dst) {
			beziers[dst]
				= Bezier(bezierPoints[src], bezierPoints[src + 1], bezierPoints[src + 2], bezierPoints[src + 3]);
		}
...
public:
	...
	{
		const Point2DList bezierPoints
			= closed() ? convertClosedToCubicBezierPoints() : convertOpenedToCubicBezierPoints();
