...
enum class CuttingDirection { FORWARD, BACKWARD };
...
{
	return (angle < 0.0f) ? (angle + std::numbers::pi_v<float> * 2.0f) : angle;
}
...
constexpr float EnsureEndGreater(float start, float end)
{
	if (end < start) {
		...
		end += std::numbers::pi_v<float> * 2.0f;
	}
...
// Explicit specialisations attach to the module of their primary template, so they must not be
// marked export; they stay reachable to any importer of this module.
...
{
	static const std::initializer_list<geometry::CuttingDirection> values
		= { geometry::CuttingDirection::FORWARD, geometry::CuttingDirection::BACKWARD };
	return values;
}
...
inline geometry::CuttingDirection fromString(const std::string& value)
{
	static const std::unordered_map<std::string, geometry::CuttingDirection> translations
		= { { "forward", geometry::CuttingDirection::FORWARD }, { "backward", geometry::CuttingDirection::BACKWARD } };
